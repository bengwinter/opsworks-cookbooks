      ...
      if options[:force] || app_config[:database][:adapter].blank?
        ...
        if options[:consult_gemfile] and File.exists?("#{app_root_path}/Gemfile")
          ...
          adapter = if bundle_list.include?('mysql2')
            ...
            Chef::Log.info("Gem mysql2 not found in the Gemfile of #{app_name}, defaulting to pg")
            'pg'
          end
        else # no Gemfile - guess adapter by Rails version
          adapter = if File.exists?("#{app_root_path}/config/application.rb")
            Chef::Log.info("Looks like #{app_name} is a Rails 3 application, defaulting to pg")
            'pg'
          else
            Chef::Log.info("No config/application.rb found, assuming #{app_name} is a Rails 2 application, defaulting to pg")
            'pg'
          end
        end
